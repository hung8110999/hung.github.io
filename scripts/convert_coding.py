"""
convert_coding.py — Markdown -> HTML Coding Post Converter

Scans coding/markdown_posts_coding/ for subfolders containing blog.md,
converts them to HTML post pages, and generates coding/coding_posts.json.

Usage: python scripts/convert_coding.py

YAML: `tag: Topic` or `tags: Algorithms, AI & ML` (comma-separated or [list]).
Also supports `repo: https://...` metadata for repository links.
"""
...
CODING_ROOT = os.path.join(ROOT, 'coding')
MARKDOWN_DIR = os.path.join(CODING_ROOT, 'markdown_posts_coding')
POSTS_DIR = os.path.join(CODING_ROOT, 'coding_posts')
POSTS_JSON = os.path.join(CODING_ROOT, 'coding_posts.json')
...
def convert_markdown(md, folder_name):
    ...
    def img_repl(m):
        ...
        final_path = f"../markdown_posts_coding/{folder_name}/{normalized}"
        inline_style = ''
...
def extract_metadata(md):
    ...
    if yaml_data:
        ...
        if 'repo' in yaml_data:
            meta['repo'] = yaml_data.get('repo', '').strip()
...
def build_post_html(meta, body_html):
    ...
    repo_html = (
        f'<p><a href="{meta["repo"]}" class="btn btn-secondary" target="_blank" rel="noopener">View Repository</a></p>'
...
<head>
    ...
    <title>{meta['title']} — Hung's Coding Notes</title>
    ...
    <main class="main-content">
        <article class="blog-post">
            ...
            <div class="blog-post-footer">
                {repo_html}
                <a href="../../coding.html" class="btn btn-secondary">← Back to Coding Notes</a>
            </div>
        </article>
    </main>
...
def main():
    ...
    for folder in folders:
        ...
        with open(out_file, 'w', encoding='utf-8') as f:
            f.write(post_html)
        print(f"[OK] Generated: coding/coding_posts/{folder}.html")
        ...
        posts_index.append({
            ...
            'tags': meta.get('tags', []),
            'repo': meta.get('repo', ''),
            'url': f'coding/coding_posts/{folder}.html',
        })
    ...
    print(f"[OK] Generated: coding/coding_posts.json with {len(posts_index)} post(s).")
