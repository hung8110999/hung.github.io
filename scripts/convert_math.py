...
YAML front matter: use `tag: Topic` or multiple labels via
  `tags: Algebra, Calculus` or `tags: [Algebra, Calculus]`.
"""
...
def _parse_tags_value(val):
    """Parse a `tags` front-matter value: comma-separated and/or [a, b] brackets."""
    if val is None:
        return []
    s = str(val).strip()
    if not s:
